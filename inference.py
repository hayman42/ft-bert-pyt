...
from modeling import BertForMaskedLM, BertForQuestionAnswering, BertConfig, WEIGHTS_NAME, CONFIG_NAME
...
def main():
    ...
    for key, value in torch.load(args.init_checkpoint, map_location='cpu').items():
        if "LayerNorm" in key:
            if "weight" in key:
                state_dict[key.replace("weight", "gamma")] = value
            if "bias" in key:
                state_dict[key.replace("bias", "beta")] = value
        else:
            state_dict[key] = value
    for i, j in zip(model.state_dict().keys(), state_dict.keys()):
        if i != j:
            print(i, j)
